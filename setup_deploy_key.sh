...
#
# This deployment runs as root (see grid-tracker.service), so these steps are
# run as root and use root's home directory ($HOME = /root). Review each
# command before running; this script is meant to be read and executed
# step-by-step, not blindly piped to bash. See DEPLOYMENT.md for the full walkthrough.
set -euo pipefail

REPO_DIR="/opt/grid-dashboard"
SSH_KEY="$HOME/.ssh/id_ed25519_grid"

# 1. Generate a dedicated ed25519 deploy key (no passphrase, for unattended use)
...
# 8. Configure committer identity for automated commits ---------------------
